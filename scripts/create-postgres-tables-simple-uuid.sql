...
-- Fonction pour générer un UUID v4 sans dépendre d'extensions ou de fonctions avancées
CREATE OR REPLACE FUNCTION generate_uuid_v4() 
RETURNS UUID AS $$
DECLARE
    v_time TIMESTAMP WITH TIME ZONE := now();
    v_macaddr TEXT := '00:00:00:00:00:00';
    v_uuid TEXT;
BEGIN
    -- Générer un UUID en utilisant uniquement des fonctions de base
    v_uuid := md5(random()::text || clock_timestamp()::text || v_time::text || v_macaddr);
    
    -- Formater en UUID v4 (RFC 4122)
    v_uuid := substring(v_uuid FROM 1 FOR 8) || '-' ||
              substring(v_uuid FROM 9 FOR 4) || '-' ||
              '4' || substring(v_uuid FROM 13 FOR 3) || '-' ||
              substring('89ab', 1 + (random() * 3)::int, 1) ||
              substring(v_uuid FROM 17 FOR 3) || '-' ||
              substring(v_uuid FROM 21 FOR 12);
...
CREATE OR REPLACE FUNCTION get_available_time_slots(space_id UUID, date DATE)
RETURNS TABLE (
  id UUID,
...
CREATE OR REPLACE FUNCTION get_bookings(user_id UUID, status TEXT DEFAULT NULL, limit_count INTEGER DEFAULT 10, offset_count INTEGER DEFAULT 0)
RETURNS TABLE (
  id UUID,
  user_id UUID,
  space_id UUID,
