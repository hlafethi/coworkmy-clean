-- Script pour vérifier manuellement et corriger la sécurité
-- À exécuter dans l'éditeur SQL de Supabase

-- 1. VÉRIFIER LA CONFIGURATION ACTUELLE DES FONCTIONS
SELECT 
...
-- 2. VÉRIFIER LES TRIGGERS ACTUELS
SELECT 
  'TRIGGERS ACTUELS' as info,
  trigger_name,
  event_object_table,
  action_timing,
  event_manipulation,
...
-- 3. SUPPRIMER TOUT ET RECRÉER PROPREMENT
-- Supprimer tous les triggers d'abord
...
-- 4. RECRÉER LES FONCTIONS AVEC ALTER FUNCTION POUR FORCER LA CONFIGURATION
...
-- Forcer la configuration du search_path
ALTER FUNCTION public.enqueue_stripe_sync() SET search_path = public, pg_temp;
...
-- Forcer la configuration du search_path
ALTER FUNCTION public.sync_space_with_stripe() SET search_path = public, pg_temp;

-- Fonction update_spaces_updated_at
...
-- 5. CRÉER UN SEUL TRIGGER UNIFIÉ
...
-- 7. VÉRIFIER LA CONFIGURATION APRÈS CORRECTION
SELECT 
  'CONFIGURATION APRÈS CORRECTION' as info,
  proname as function_name,
  pronargs as argument_count,
  prosecdef as security_definer,
  proconfig as function_config,
  ...
  CASE 
    ...
    WHEN proconfig @> ARRAY['search_path=public,pg_temp,pg_catalog'] THEN '✅ Sécurisé'
    WHEN proconfig IS NOT NULL AND array_length(proconfig, 1) > 0 THEN '⚠️ Configuré mais format inattendu'
    ELSE '❌ Non sécurisé'
...
-- 8. VÉRIFIER LES TRIGGERS FINAUX
SELECT 
...
BEGIN
    ...
    RAISE NOTICE 'Vérifiez les résultats ci-dessus pour la configuration';
END $$; 