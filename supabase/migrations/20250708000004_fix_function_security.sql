-- Migration pour corriger la sécurité des fonctions
...
-- 2. Corriger la fonction sync_space_with_stripe si elle existe
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'sync_space_with_stripe') THEN
    EXECUTE '
...
-- 3. Corriger la fonction sync_space_with_stripe(UUID) si elle existe
DO $$
BEGIN
...
-- 4. Vérifier que toutes les fonctions ont un search_path sécurisé
SELECT 
  'FONCTIONS AVEC SEARCH_PATH SÉCURISÉ' as info,
  proname as function_name,
  prosrc as function_source
FROM pg_proc 
WHERE proname IN ('enqueue_stripe_sync', 'sync_space_with_stripe')
