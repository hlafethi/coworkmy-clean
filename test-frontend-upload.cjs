...
async function testFrontendUpload() {
  try {
    console.log('🔍 Test de l\'upload côté frontend...\n');
    ...
    // 2. Simuler un upload avec document_type "identity" (comme le ferait le frontend)
    console.log('\n2️⃣ Upload avec document_type "identity"...');
    ...
    const uploadData = {
      file_name: 'test-identity-frontend.pdf',
      file_type: 'application/pdf',
      file_size: 100,
      file_content: testBase64,
      document_type: 'identity' // Type spécifique comme le frontend l'envoie
    };

    console.log('📤 Données d\'upload (comme le frontend):', {
    ...
    // 3. Vérifier immédiatement en base de données
    ...
    if (getResponse.data.length > 0) {
      ...
      if (latestDoc.document_type === 'identity') {
        ...
        console.log('🔍 Attendu: identity, Reçu:', latestDoc.document_type);
      }
    }
