-- Correction du trigger pour éviter la boucle infinie
-- Le trigger ne doit se déclencher que sur les champs métier, pas sur les champs Stripe

-- 1. Fonction trigger corrigée pour éviter la boucle
...
-- 2. Nettoyer la queue existante pour éviter les jobs en boucle
DELETE FROM public.stripe_sync_queue WHERE status = 'pending';

-- 3. Log de la correction
DO $$
BEGIN
  RAISE NOTICE 'Trigger stripe_sync corrigé pour éviter la boucle infinie';
END $$; 