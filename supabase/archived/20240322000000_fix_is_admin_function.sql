-- Suppression robuste des policies dépendantes de is_admin()
DO $$ BEGIN
...
-- Recréer les policies supprimées
CREATE POLICY "Écriture admin homepage" ON public.homepage_settings
...
CREATE POLICY "Les administrateurs peuvent voir les utilisateurs admin" ON public.admin_users FOR SELECT USING (public.is_admin());
CREATE POLICY "Seuls les administrateurs peuvent modifier la configuration" ON public.google_business_config FOR ALL USING (public.is_admin());
CREATE POLICY "Les administrateurs peuvent gérer les promotions" ON public.promotions FOR ALL USING (public.is_admin());
CREATE POLICY "Les administrateurs peuvent gérer les équipements" ON public.space_equipment FOR ALL USING (public.is_admin());
CREATE POLICY "Les administrateurs peuvent voir toutes les notifications" ON public.notifications FOR SELECT USING (public.is_admin());
...
CREATE POLICY "Les administrateurs peuvent voir toutes les factures" ON public.invoices FOR SELECT USING (public.is_admin());
CREATE POLICY "Seuls les administrateurs peuvent voir les logs" ON public.application_logs FOR ALL USING (public.is_admin());
CREATE POLICY "Seuls les administrateurs peuvent voir les logs" ON public.email_logs FOR ALL USING (public.is_admin());
