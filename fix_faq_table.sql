-- Script de nettoyage et correction pour la table FAQ
...
-- 3. Supprimer la fonction si elle existe (pour éviter les conflits)
DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;

-- 4. Recréer la fonction update_updated_at_column
...
-- Politique pour permettre la gestion complète aux admins (utilise la fonction is_admin existante)
CREATE POLICY "FAQ - gestion complète admin" ON support_faqs
    FOR ALL USING (public.is_admin());

-- 7. Vérifier que la table a des données, sinon insérer les FAQ par défaut
