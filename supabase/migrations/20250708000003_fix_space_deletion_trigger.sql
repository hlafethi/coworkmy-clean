-- Migration pour corriger le problème de suppression d'espace
-- Le trigger essaie d'insérer NULL dans space_id lors de la suppression
-- Solution : corriger la logique du trigger pour gérer correctement les suppressions

-- 1. Corriger la fonction trigger pour gérer correctement les suppressions
...
-- 2. Nettoyer les enregistrements orphelins dans stripe_sync_queue
-- Supprimer les enregistrements qui référencent des espaces qui n'existent plus
...
DO $$
BEGIN
    RAISE NOTICE 'Trigger de suppression d''espace corrigé avec succès';
